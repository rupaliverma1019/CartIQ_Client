...
const AISearch = () => {
  ...
  const handleSearch = async (searchText = prompt) => {
    if (!searchText.trim()) {
      setError("Please enter a search prompt.");
      return;
    }
    ...
    try {
      ...
      const data = await searchAIProducts(searchText);
      ...
      setHistory((prev) => [
        searchText,
        ...prev.filter((item) => item !== searchText).slice(0, 4),
      ]);
    } catch (error) {
      setProducts([]);
      setKeywords("");
